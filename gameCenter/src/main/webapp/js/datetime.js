(function(){
		...
		for(var md=mon[date['m']-1],wd=md-date['w']+1,n=0;n<6;n++){
			...
			for(var nn=0;nn<7;nn++){
				if(wd<=md){
					...
					if(day<=mon[date['m']]){
						if(day==testdate['d'] && date['m'] == testdate['m'] && date['y']==testdate['y']){
	...
	function show(o){
		...
		var p=getPos(o);
		if(s){
			var t=/(\d+)-(\d+)-(\d+)\s*(\d*):?(\d*):?(\d*)/.exec(s);
			var da=new Date(t[1],t[2]-1,t[3],t[4],t[5],t[6]);
			testdate = getdate(da)
			formatDay(s);
		}else{
			now=new Date();
	...
	function getObj(className){
		var o=d.getElementsByTagName('input'),oArr=[];
		for(var i=0,len=o.length;i<len;i++){
			if(o[i].className.indexOf(className) > -1){
				oArr.push(o[i])
			}
		}
		return oArr;
