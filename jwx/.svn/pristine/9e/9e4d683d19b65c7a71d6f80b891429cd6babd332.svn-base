...
function addGroupMember(title, addurl, gname, width, height) {
    gridname = gname;
    creategroupmemberwindow(title, addurl, width, height);
}
 ...
 * @return
 */
function deleteALLGroupMember(title, url, gname) {
    $.dialog.setting.zIndex = 1980;
    gridname = gname;
    var ids = [];
    var rows = $("#" + gname).datagrid('getSelections');
...
function addALLSelect(title, url, gname) {
    ...
    var rows = $("#" + gname).datagrid('getSelections');
    if (rows.length > 0) {
        //$.dialog.confirm('你确定添加该数据吗?',
        //function (r) {
        //     if (r) {

        //window.confirm('你确定添加该数据吗?',function (r){
...
function change(url, name) {
    gridname = name;
    createdialog('更改确认 ', '确定更改此接口的设置吗 ?', url, name);
}
...
function creategroupmemberwindow(title, addurl, width, height) {
    width = width ? width : 700;
    height = height ? height : 400;
    if (width == "100%" || height == "100%") {
        width = window.top.document.body.offsetWidth;
 ...
 */
function doAjaxSubmit(url, callback) {
