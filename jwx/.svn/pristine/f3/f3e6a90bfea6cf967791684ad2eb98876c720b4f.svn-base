...
public class NewsItemController {
	...
	public ModelAndView newscontent(HttpServletRequest request){
		...
		NewsItem newsItem = this.newsItemService.getEntity(NewsItem.class, id);
        //增加阅读数
		if(newsItem.getReadNumber() == null){
			newsItem.setReadNumber(0);
		Integer number = newsItem.getReadNumber() + 1;
		newsItem.setReadNumber(number);
		newsItemService.saveOrUpdate(newsItem);
		}else {
		 Integer number = newsItem.getReadNumber() + 1;
		 newsItem.setReadNumber(number);
		 newsItemService.saveOrUpdate(newsItem);
