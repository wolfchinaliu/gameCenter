	...
	<tbody id="add_cgreportConfigItem_table">
		<c:if test="${fn:length(cgreportConfigItemList)  <= 0 }">
			<tr>
				...
				<td align="left"><t:dictSelect field="cgreportConfigItemList[0].fieldType"  extendJson="{style:'width:80px'}" type="list" typeGroupCode="fieldtype" defaultVal="${cgreportConfigItemPage.fieldType}" hasLabel="false" title="字段类型"></t:dictSelect></td>
				...
				<td align="left"><t:dictSelect field="cgreportConfigItemList[0].SMode" type="list"  extendJson="{style:'width:90px'}" typeGroupCode="searchmode" defaultVal="${cgreportConfigItemPage.sMode}" hasLabel="false" title="查询模式"></t:dictSelect></td>
				...
				<td align="left"><t:dictSelect field="cgreportConfigItemList[0].SFlag"  extendJson="{style:'width:60px'}" divClass="STYLE_LEG" type="list" typeGroupCode="yesorno" defaultVal="${cgreportConfigItemPage.sFlag}" hasLabel="false" title="是否查询"></t:dictSelect></td>
			</tr>
		</c:if>
		<c:if test="${fn:length(cgreportConfigItemList)  > 0 }">
			<c:forEach items="${cgreportConfigItemList}" var="poVal" varStatus="stuts">
				<tr>
					...
					<td align="left"><t:dictSelect field="cgreportConfigItemList[${stuts.index }].fieldType" type="list" extendJson="{style:'width:80px'}" typeGroupCode="fieldtype" defaultVal="${poVal.fieldType}" hasLabel="false" title="字段类型"></t:dictSelect>
					</td>
					...
					<td align="left"><t:dictSelect field="cgreportConfigItemList[${stuts.index }].SMode"  extendJson="{style:'width:90px'}" type="list" typeGroupCode="searchmode" defaultVal="${poVal.SMode}" hasLabel="false" title="查询模式"></t:dictSelect></td>
					...
					<td align="left"><t:dictSelect field="cgreportConfigItemList[${stuts.index }].SFlag" extendJson="{style:'width:60px'}" type="list" typeGroupCode="yesorno" defaultVal="${poVal.SFlag}" hasLabel="false" title="是否查询"></t:dictSelect></td>
				</tr>
			</c:forEach>
		</c:if>
	</tbody>
