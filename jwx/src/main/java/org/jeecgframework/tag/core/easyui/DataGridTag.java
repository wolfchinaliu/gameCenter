...
public class DataGridTag extends TagSupport {
	...
	public StringBuffer end() {
		...
		sb.append("</script>");
		//sb.append("<table width=\"100%\" height:\"100%\"  id=\"" + name + "\" toolbar=\"#" + name + "tb\"></table>");
		
		sb.append("<table width=\"100%\" height:\"100%\"  id=\"" + name + "\" toolbar=\"#" + name + "tb\" border=\"0\"></table>");
		
		//sb.append("<div id=\"" + name + "tb\" style=\"padding:3px; height: auto\" >");
		
		sb.append("<div id=\"" + name + "tb\" style=\"height:30px;\"  border=\"0\" >");
		
		sb.append("<div name=\"searchColums\" style=\"padding-top:2px;\" >");
		
		sb.append("<span style=\"float:left;vertical-align:middle;margin-left:5px;\" >");
		...
		if(hasQueryColum(columnList)){
			...
			if("group".equals(getQueryMode())){
				for (DataGridColumn col : columnList) {
					if (col.isQuery()) {
						sb.append("<span style=\"padding-bottom:3px;vertical-align:middle;display:-moz-inline-box;display:inline-block;margin-left:5px;\">");
						sb.append("<span style=\"padding-bottom:4px;margin-right:5px;vertical-align:middle;display:-moz-inline-box;display:inline-block;width:auto;text-align:right;text-overflow:ellipsis;-o-text-overflow:ellipsis; overflow: hidden;white-space:nowrap; \" title=\""+col.getTitle()+"\">"+col.getTitle()+"</span>");
						
						
						if("single".equals(col.getQueryMode())){
							if(!StringUtil.isEmpty(col.getReplace())){
								sb.append("<select name=\""+col.getField().replaceAll("_","\\.")+"\" WIDTH=\"80\" style=\"width: 84px\"> ");
								...
								sb.append("<input onkeypress=\"EnterPress(event)\" onkeydown=\"EnterPress()\" type=\"text\" name=\""+col.getField().replaceAll("_","\\.")+"\"  "+extendAttribute(col.getExtend())+" style=\"width: 80px\" />");
								
							}
						}else if("group".equals(col.getQueryMode())){
							sb.append("<input type=\"text\" name=\""+col.getField()+"_begin\"  style=\"width: 84px\" "+extendAttribute(col.getExtend())+"/>");
							sb.append("<span style=\"display:-moz-inline-box;display:inline-block;width: 8px;text-align:right;\">~</span>");
							sb.append("<input type=\"text\" name=\""+col.getField()+"_end\"  style=\"width: 84px\" "+extendAttribute(col.getExtend())+"/>");
						}
			...
			if("group".equals(getQueryMode()) && hasQueryColum(columnList)){
				sb.append("<span style=\"margin-left:5px;margin-right:5px;\" >");
				sb.append("<a href=\"#\" class=\"easyui-linkbutton\" plain=\"true\" iconCls=\"icon-search\" onclick=\""+name+"search()\">查询</a>");
				sb.append("<a href=\"#\" class=\"easyui-linkbutton\" plain=\"true\" iconCls=\"icon-reload\" onclick=\"searchReset('"+name+"')\">重置</a>");
				sb.append("</span>");
			}
...
//		}else{
//			sb.append("<div style=\"height:30px;vertical-align:middle;padding-top:2px;\" >");
//		}
...
//		sb.append("</span>");
		if("group".equals(getQueryMode()) && hasQueryColum(columnList)){//如果表单是组合查询
			//sb.append("<span style=\"float:right\">");
			//sb.append("<a href=\"#\" style=\"margin-right:3px;\" class=\"easyui-linkbutton\" iconCls=\"icon-search\" onclick=\""+name+"search()\">查询</a>");
			//sb.append("<a href=\"#\" style=\"margin-right:1px;\" class=\"easyui-linkbutton\" iconCls=\"icon-reload\" onclick=\"searchReset('"+name+"')\">重置</a>");
			//sb.append("</span>");
	...
	/**
	 * 尝试在message中可以显示参数，未果！dyt
	 * */
//	protected String formatMassage(String message) {
//        Pattern pattern = Pattern.compile("\\{.*?\\}");
//        Matcher matcher = pattern.matcher(message);
//        while(matcher.find()){
//            String match = matcher.group(0);
//            String key = match.substring(1, match.length()-1);
//            message = message.replace(match, "rec."+key+"");
//        }
//        return message;
//    }
	/**
