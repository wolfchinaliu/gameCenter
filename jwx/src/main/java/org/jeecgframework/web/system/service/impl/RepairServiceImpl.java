...
//		commonDao.saveOrUpdate(jform_cgreport_item_field_type);
	...
	private void repairUser() {
		...
		TSRole role = commonDao.findUniqueByProperty(TSRole.class, "roleCode", "admin");
		TSRole tyrole = commonDao.findUniqueByProperty(TSRole.class, "roleCode", "01");
	...
	private void repairAttachment() {
		...
		settable.setAttachmenttitle("（张三）-金融服务公司机构岗位职责与任职资格设置表(根据模板填写）");
		settable.setRealpath("（张三）-金融服务公司机构岗位职责与任职资格设置表(根据模板填写）");
