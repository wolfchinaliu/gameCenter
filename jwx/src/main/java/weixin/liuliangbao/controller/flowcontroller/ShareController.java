...
import weixin.liuliangbao.jsonbean.*;
...
import weixin.util.PropertyUtils;
...
import static weixin.guanjia.openplatform.controller.WeixinOpenPlatformController.AccountIdCache;
import static weixin.guanjia.openplatform.controller.WeixinOpenPlatformController.PageAuthPropertyCache;
...
public class ShareController extends BaseController {
    ...
    public ModelAndView setShare(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            buffer.append("shareController_setShare_begin:_");
...
//        String url = "http://localhost:8080/jwx/share/QueryActicle";
    ...
    public AjaxJson doUpdate(ShareItem shareItem, HttpServletRequest request) throws BusinessException {
        ...
        long startTime = System.currentTimeMillis();
        StringBuffer buffer = new StringBuffer();
        buffer.append("shareController_setShare_doUpdate_begin:_");
        try {
...
//            shareItem.setId(id);
    ...
    /**
     * http://weixina.1shiliu.com/jwx/shareController.do?startLoad&accountid=ea84c08653c7aeb30153cbb2288f06d8&shareId=ea84c086555e5224015561c7d2dc0e84
     * 用户自己点击链接进入分享流量页面
     *
     * @param request
     * @param response
    ...
    public ModelAndView showSharePage(HttpServletRequest request) throws Exception {
        CommonUtils.printRequestParam("ShareController.showSharePage", request);
        long startTime = System.currentTimeMillis();//获取开始当前的时间
        StringBuffer buffer = new StringBuffer();
        buffer.append("shareController_showSharePage_begin_");
        ...
        try {
            ...
            if (StringUtils.isBlank(type1)) {
                ...
                if (StringUtils.isBlank(appid)) {
                    LOGGER.info("shareController_showSharePage普通1公众号网页授权");
                } else {
                    LOGGER.info("shareController_showSharePage第三3方平台网页授权");
                    authType = 1;
                }
            ...
            if (shareItem != null) {
                ...
                URIBuilder builder = new URIBuilder("shareController.do?forwardShare");
                JSONObject params = new JSONObject();
                params.put("openId", StringUtils.defaultIfBlank(null == memberEntity ? "" : memberEntity.getOpenId(), params.getString("openId")));
                ...
                if (memberEntity == null || memberEntity.getSubscribe().equals("0")) {
                    ...
                    buffer.append("_noSub_").append("shareController_showSharePage_end_time:" + (endTime - startTime) + "ms");
                    LOGGER.info(buffer.toString());
                    ...
                    if (StringUtils.isBlank(memberEntity.getPhoneNumber())) {
                        // 没有绑定，跳到点击按钮弹出绑定的页面
                        long endTime = System.currentTimeMillis();//获取结束的当前时间
                        buffer.append("_noBinding_").append("shareController_showSharePage_end_time:" + (endTime - startTime) + "ms");
                        LOGGER.info(buffer.toString());
                        return new ModelAndView("redirect:" + forwardUrl);
                    } else {
                        // 绑定了，跳到只有一个按钮的页面，允许分享
                        long endTime = System.currentTimeMillis();//获取结束的当前时间
                        buffer.append("_share_").append("shareController_showSharePage_end_time:" + (endTime - startTime) + "ms");
                        LOGGER.info(buffer.toString());
                        return new ModelAndView("redirect:" + forwardUrl);
                    }
                }
                ...
                long endTime = System.currentTimeMillis();
                buffer.append("_noShare_").append("shareController_showSharePage_end_time:" + (endTime - startTime) + "ms");
                LOGGER.info(buffer.toString());
    ...
    public ModelAndView forwardSharePage(HttpServletRequest request) throws Exception {
        CommonUtils.printRequestParam("ShareController.forwardSharePage", request);
    ...
    /**
     * 查看人点击分享的链接时, 调用此接口
     * http://weixina.1shiliu.com/jwx/shareController.do?load
    ...
    public ModelAndView load(HttpServletRequest request, HttpServletResponse response) throws Exception {
        CommonUtils.printRequestParam("ShareController.load", request);
        String shareId = request.getParameter("shareId");
        try {
    ...
    public void share(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        ShareRecordEntity shareRecord  = new ShareRecordEntity();
        shareRecord.setAccountName(accountName);
        shareRecord.setAccountid(accountid);
        shareRecord.setShareId(shareId);
        shareRecord.setReason("2");
        shareRecord.setTitle(title);
        shareRecord.setCreatetime(new Date());
        this.systemService.save(shareRecord);
