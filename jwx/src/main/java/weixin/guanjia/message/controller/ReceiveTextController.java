...
public class ReceiveTextController {
    public static final transient Logger LOGGER = Logger.getLogger(ReceiveTextController.class);
	...
	@ResponseBody
	public void datagrid(ReceiveText receiveText, HttpServletRequest request, HttpServletResponse response,
			DataGrid dataGrid) {
		...
		if (createTime_begin != null&&!"".equals(createTime_begin)) {
			Timestamp beginValue = null;
			try {
				beginValue = DataUtils.parseTimestamp(createTime_begin, "yyyy-MM-dd");
		...
		if (createTime_end != null&&!"".equals(createTime_end)) {
			if (createTime_end.length() == 10) {
				createTime_end = createTime_end + " 23:59:59";
			}
			Timestamp endValue = null;
			try {
				endValue = DataUtils.parseTimestamp(createTime_end, "yyyy-MM-dd hh:mm:ss");
			} catch (ParseException e) {
				e.printStackTrace();
			}
