...
List
<${entityName}Entity> ${entityName?uncap_first}s = this.${entityName?uncap_first}
    Service.getListByCriteriaQuery(cq,false);
    workbook = ExcelExportUtil.exportExcel(new ExcelTitle("${ftl_description}列表",
    ...
    */
    ...
    public void exportXlsByT(${entityName}Entity ${entityName?uncap_first},HttpServletRequest
    ...
    Map
    <String
            , MultipartFile> fileMap = multipartRequest.getFileMap();
        for (Map.Entry
        <String
                , MultipartFile> entity : fileMap.entrySet()) {
            ...
            try {
            List
            <${entityName}Entity> list${entityName}Entitys =
                (List
                <${entityName}Entity>)ExcelImportUtil.importExcelByIs(file.getInputStream(),${entityName}
                    Entity.class,params);
