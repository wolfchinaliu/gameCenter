(function ($) {
    ...
    window.mainpage.loadMenu = function (id) {
        ...
        $('#navMenu_Tree').tree({
            url: 'loginController.do?menuTree'
        });
...
//            $.get(id == 11 ? "easyui/common/nav-api-menu-data.json" : "easyui/common/nav-doc-menu-data.json", function (menus) {
...
//        }
    ...
    window.mainpage.loadNavMenus = function (callback) {
        ...
        $.get("easyui/common/nav-menu-data.json", function (menus) {
            ...
            contextMenu: [
                {
                    ...
                    disabled: function (e, node) {
                        return node.attributes && node.attributes.href ? false : true;
                    },
                ...
                {
                    text: "添加至个人收藏", iconCls: "icon-standard-feed-add", disabled: function (e, node) {
                    return !t.tree("isLeaf", node.target);
                }, handler: function (e, node) {
                    window.mainpage.nav.addFavo(node.id);
                }
                },
                //{ text: "重命名", iconCls: "icon-hamburg-pencil", handler: function (e, node) { t.tree("beginEdit", node.target); } }, "-",
                {
                    text: "刷新", iconCls: "icon-cologne-refresh", handler: function (e, node) {
                    window.mainpage.nav.refreshTree();
                }
                }
            ],
            onAfterEdit: function (node) {
                window.mainpage.nav.rename(node.id, node.text);
            }
        });
    };
        ...
        t.tree({
            ...
            onBeforeDrop: function (target, source, point) {
                ...
                if (point == "append" || !point) {
                    if (!node || !node.attributes || !node.attributes.folder) {
                        return false;
                    }
                }
            },
            ...
            contextMenu: [
                ...
                {
                    text: "从个人收藏删除", iconCls: "icon-standard-feed-delete", handler: function (e, node) {
                    window.mainpage.favo.removeFavo(node.id);
                }
                },
                //{ text: "重命名", iconCls: "icon-hamburg-pencil", handler: function (e, node) { t.tree("beginEdit", node.target); } }, "-",
                {
            ...
            onAfterEdit: function (node) {
                window.mainpage.favo.rename(node.id, node.text);
            }
        });
    };
    ...
    window.mainpage.loadFavoMenus = function () {
        ...
        //$(favoMenuTree).tree("load", "easyui/common/favo-menu-data.json");
        $(favoMenuTree).tree("load", "loginController.do?favoMenu");
        //$('#favoMenu_Tree').tree({
        //    url:'loginController.do?favoMenu'
        //});
    };
    ...
    window.mainpage.instTimerSpan = function () {
        var timerSpan = $("#timerSpan"), interval = function () {
    ...
    window.mainpage.bindToolbarButtonEvent = function () {
        ...
        $("#btnHideNorth").click(function () {
            window.mainpage.hideNorth();
        });
        var btnShow = $("#btnShowNorth").click(function () {
            window.mainpage.showNorth();
        });
        ...
        //        width: 600,
        //        height: 360,
        //        href: 'userController.do?userinfo',
        //        topMost: false,
        //        modal:true,
        //        buttons:[{
        //		text:'确定',
        ...
        //
        //$(btn2).click(function () {
        //    window.open("helpController.do?list", "_blank");
        //});
        ...
        $(btn3).click(function () {
            $('#dd').dialog({
                ...
                width: 600,
                height: 360,
                href: 'userController.do?changepassword',
                topMost: false,
                modal: true,
                buttons: [{
                    text: '确定',
        ...
        $(btnKey).click(function () {
            $('#dd').dialog({
                ...
                width: 500,
                height: 200,
                href: 'userController.do?goSecretKey',
                topMost: false,
                modal: true,
              buttons: null
//                          {
...
//                    }]
        ...
        $(btnExit).click(function () {
            $.easyui.messager.confirm("操作提醒", "您确定要退出系统并关闭该网页？", function (c) {
                if (c) {

                    $.ajax({
                        type: 'POST',
                        url: 'loginController.do?loginout',
                        dataType: 'json',
    ...
    window.mainpage.search = function (value, name) {
        $.easyui.messager.show($.string.format("您设置的主题为：value: {0}, name: {1}", value, name));
    };
    ...
    window.mainpage.setTheme = function (theme, setCookie) {
        ...
        $.easyui.theme(true, theme, function (item) {
            if (setCookie) {
                ...
                var msg = $.string.format("您设置了新的系统主题皮肤为：{0}", item.name);
                //var msg = $.string.format("您设置了新的系统主题皮肤为：{0}，{1}。", item.name, item.path);
                //$.easyui.messager.show(msg);
                $.easyui.messager.show({msg: msg, position: "bottomRight"});
            }
        });
    };
    ...
    window.mainpage.bindMainTabsButtonEvent = function () {
        ...
        $("#mainTabs_jumpTab").click(function () {
            window.mainpage.mainTabs.jumpTab();
        });
    ...
    window.mainpage.bindNavTabsButtonEvent = function () {
        ...
        $("#navMenu_Favo").click(function () {
            window.mainpage.nav.addFavo();
        });
        $("#navMenu_Rename").click(function () {
            window.mainpage.nav.rename();
        });
        $("#navMenu_expand").click(function () {
            window.mainpage.nav.expand();
        });
        $("#navMenu_collapse").click(function () {
            window.mainpage.nav.collapse();
        });
        $("#navMenu_collapseCurrentAll").click(function () {
            window.mainpage.nav.expandCurrentAll();
        });
        $("#navMenu_expandCurrentAll").click(function () {
            window.mainpage.nav.collapseCurrentAll();
        });
        $("#navMenu_collapseAll").click(function () {
            window.mainpage.nav.expandAll();
        });
        $("#navMenu_expandAll").click(function () {
            window.mainpage.nav.collapseAll();
        });

        $("#favoMenu_Favo").click(function () {
            window.mainpage.favo.removeFavo();
        });
        $("#favoMenu_Rename").click(function () {
            window.mainpage.favo.rename();
        });
        $("#favoMenu_expand").click(function () {
            window.mainpage.favo.expand();
        });
        $("#favoMenu_collapse").click(function () {
            window.mainpage.favo.collapse();
        });
        $("#favoMenu_collapseCurrentAll").click(function () {
            window.mainpage.favo.expandCurrentAll();
        });
        $("#favoMenu_expandCurrentAll").click(function () {
            window.mainpage.favo.collapseCurrentAll();
        });
        $("#favoMenu_collapseAll").click(function () {
            window.mainpage.favo.expandAll();
        });
        $("#favoMenu_expandAll").click(function () {
            window.mainpage.favo.collapseAll();
        });
    };

    window.mainpage.hideNorth = function () {
        $(mainLayout).layout("collapse", "north");
    };

    window.mainpage.showNorth = function () {
    ...
    window.mainpage.mainTabs.addModule = function (title, href, iconCls, iniframe, closable, refreshable, selected) {
        ...
        switch (isExists) {
            ...
            case 1:
                ...
                break;
            case 2:
                window.mainpage.mainTabs.jumeTab(opts.title);
                break;
            default:
                break;
        }
    ...
    window.mainpage.mainTabs.jumeTab = function (which) {
...
//        $.get("easyui/common/nav-api-menu-data.json", function (menus) {
...
//        }, "json");
    ...
    window.mainpage.nav.refreshNav = function () {
    ...
    window.mainpage.nav.expand = function (id) {
        ...
        if (id == null || id == undefined) {
            ...
            node = t.tree("find", id);
            if (!node) {
                return $.easyui.messager.show("请传入有效的参数 id(菜单标识号)");
            }
        }
    ...
    window.mainpage.nav.expandAll = function () {
        $(navMenuTree).tree("expandAll");
    };

    window.mainpage.nav.collapseAll = function () {
        $(navMenuTree).tree("collapseAll");
    };


    window.mainpage.favo.refreshTree = function () {
        window.mainpage.loadFavoMenus()
    };
    ...
    window.mainpage.favo.removeFavo = function (id) {
        ...
        $.ajax({
            type: 'POST',
            url: 'tFavoMenuController.do?delFavoMenu',
            data: {menu_id: node.id},
            dataType: 'json',
            success: function (data) {
    ...
    window.mainpage.favo.addFolder = function (node) {
        ...
        $.easyui.messager.prompt("请输入添加的目录名称：", function (name) {
            ...
            if (String(name).trim() == "") {
                return $.easyui.messager.show("请输入目录名称！");
            }
            if (node) {
                t.tree("insert", {
                    after: node.target,
                    data: {id: folderId++, text: name, iconCls: "icon-hamburg-folder", attributes: {folder: true}}
                });
            } else {
    ...
    window.mainpage.favo.expandAll = function () {
        $(favoMenuTree).tree("expandAll");
    };

    window.mainpage.favo.collapseAll = function () {
    ...
    window.donate.reload = function () {
        ...
        $.ajax({
            url: "easyui/common/donate-data.json", type: "get", dataType: "json",
            beforeSend: function (XMLHttpRequest) {
                $.easyui.loading({locale: donatePanel});
            },
    ...
    window.link.reload = function () {
        ...
        $.ajax({
            url: "easyui/common/link-data.json", type: "get", dataType: "json",
            beforeSend: function (XMLHttpRequest) {
                $.easyui.loading({locale: linkPanel});
            },
            success: function (data, textStatus, XMLHttpRequest) {
