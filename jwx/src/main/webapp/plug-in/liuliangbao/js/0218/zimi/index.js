        ...
        if (lenNo >= len) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    function reWriteLiuliang(){

                        var $guo = $("#j-liuliang-guo");
                        var $shen = $("#j-liuliang-shen");

                        if($('#provinceAccount').val() == "全国任意地区"){

                            $guo.text( parseFloat($guo.text(),10) + data.attributes.totalFlowGet);
                        }
                        else {
                            $shen.text( parseFloat($shen.text(),10) + data.attributes.totalFlowGet);
                        }
                    ...
                    } else if (data.msg == "noPhoneNumber") {
                        Zepto('body').popup({
                            title: '提示',
                            ...
                            onOk: function () {
                                // 确认按钮的回调函数
                                bindPhoneNumber(accountId, openId, nickName, phoneNumber, "猜灯谜");
                            },
                            cnacel2: "取消",
                            onCancel2: function () {
                                reWriteLiuliang();
