...
<html>
 ...
 <script type="text/javascript">
 	$(document).ready(function(){ 
 	   ...
 	   if(templateId){
	 		...
	 		templateObj.html("");
	 		$.ajax({
	 			url:"customMenuManagerController.do?gettemplate",
 	   ...
 	   if(true){
	 		...
	 		supMenuIdObj.html("");
	 		$.ajax({
	 			url:"customMenuManagerController.do?getSubMenu",
	 			data:{"msgType":msgType},
	 			dataType:"JSON",
	 			type:"POST",
	 			success:function(data){
 ...
 <div id="point_title" style="border: solid #f7cc8f 1px;background: #FFEFCE;margin-bottom: 5px;margin-top: 5px;">
 	<p style="padding-left: 15px;">说明：当您为自定义菜单填写网页链接地址时请填写以"http://"开头，这样可以保证用户手机浏览的兼容性更好
 	<span style="float: right;padding-right: 5px;"><a href="#" onclick="closePoint();">关闭提示</a></span></p>
 </div>
</c:if>
 
  <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="customMenuManagerController.do?su">
   ...
   <table style="width:100%" cellpadding="0" cellspacing="1" class="formtable">
    ...
    <tr>
     ...
     <td colspan="3" class="value">
      <select name="type" id="type">
      	...
      	<option value="click"  <c:if test="${type=='click'}">selected="selected"</c:if>>消息触发</option>
      	<option value="view"  <c:if test="${type=='view'}">selected="selected"</c:if>>网页链接</option>
      </select>
    ...
    <tr id="xxtr" style="width:100px">
     ...
     <td class="value" colspan="3">
        ...
        <input type="radio" value="expand" name="msgType"  <c:if test="${msgType=='expand'}">checked="checked"</c:if>/>扩展功能
      <span class="Validform_checktip">选择消息类型</span>
     </td>
    </tr>
    
    <tr id="templateTr" style="width:100px">
     <td align="right">
      <label class="Validform_label">
       选择模板:
      </label>
